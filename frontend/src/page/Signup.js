...
console.log(process.env.REACT_APP_SERVER_DOMIN);
    const handleSubmit = async(e) => {
        ...
        if (firstName && email && password && confirmPassword) {
            if (password === confirmPassword) {
                ...
                toast('Password and Confirm password was wrong!!')
            }
        }else{
  ...
  return (
    <div className='p-3 md:p-4'>
        <div className='w-fuul max-w-sm bg-white m-auto flex items-center flex-col p-4'>
            ...
            <form className='w-full py-3 flex flex-col' onSubmit={handleSubmit}>
                ...
                <button className='w-full max-w-[100px] m-auto bg-red-500 hover:bg-red-600 cursor-pointer text-white text-xl font-medium text-center py-1 rounded-full mt-4'>Sign up</button>
            </form>
            <p className='text-sm mt-2'>Already have account? <Link to={"/login"} className='text-red-500 underline'>Login</Link></p>
        </div>
    </div>
  )
}
